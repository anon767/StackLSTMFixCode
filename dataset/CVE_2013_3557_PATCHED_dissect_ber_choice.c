...
printf("CHOICE CVE_2013_3557_PATCHED_dissect_ber_choice(%s) entered offset:%d len:%d %02x:%02x:%02x\n",name,offset,tvb_length_remaining(tvb,offset),tvb_get_guint8(tvb,offset),tvb_get_guint8(tvb,offset+1),tvb_get_guint8(tvb,offset+2));
}else{
printf("CHOICE CVE_2013_3557_PATCHED_dissect_ber_choice(%s) entered len:%d\n",name,tvb_length_remaining(tvb,offset));
}
}
#endif
    start_offset=offset;
    ...
    if(hf_id >= 0){
        ...
        case FT_UINT32:
            break;
        default:
            proto_tree_add_text(tree, tvb, offset, len,"CVE_2013_3557_PATCHED_dissect_ber_choice(): Was passed a HF field that was not integer type : %s",hfinfo->abbrev);
            fprintf(stderr,"CVE_2013_3557_PATCHED_dissect_ber_choice(): frame:%u offset:%d Was passed a HF field that was not integer type : %s\n",actx->pinfo->fd->num,offset,hfinfo->abbrev);
            return end_offset;
        }
    }
...
printf("CHOICE CVE_2013_3557_PATCHED_dissect_ber_choice(%s) calling subdissector start_offset:%d offset:%d len:%d %02x:%02x:%02x\n",name,start_offset,offset,tvb_length_remaining(next_tvb,0),tvb_get_guint8(next_tvb,0),tvb_get_guint8(next_tvb,1),tvb_get_guint8(next_tvb,2));
}else{
printf("CHOICE CVE_2013_3557_PATCHED_dissect_ber_choice(%s) calling subdissector len:%d\n",name,tvb_length(next_tvb));
}
}
#endif
            if (next_tvb == NULL) {
...
printf("CHOICE CVE_2013_3557_PATCHED_dissect_ber_choice(%s) subdissector ate %d bytes\n",name,count);
}
#endif
...
printf("CHOICE CVE_2013_3557_PATCHED_dissect_ber_choice(%s) trying again\n",name);
}
#endif
