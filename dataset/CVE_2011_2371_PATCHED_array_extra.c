static JSBool
CVE_2011_2371_PATCHED_array_extra(JSContext *cx, ArrayExtraMode mode, uintN argc, Value *vp)
{
    ...
    newarr = NULL;
#endif
    uint64 start = 0, end = length, step = 1;

    switch (mode) {
      case REDUCE_RIGHT:
        start = length - 1, end = UINT64_MAX, step = UINT64_MAX;
        /* FALL THROUGH */
      case REDUCE:
        if (length == 0 && argc == 1) {
